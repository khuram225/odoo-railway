...
MODEL_XMLID_RE = re.compile(r'^model_[a-z0-9_]+$')
    ...
    for f in files:
        ...
        for m in BLOCK_RE.finditer(text):
            ...
            for r in refs:
                ...
                if MODEL_XMLID_RE.match(r):
                    # model_<model_name> is generated by Odoo itself while
                    # reflecting the models (ir_model.py's model_xmlid(),
                    # called from _reflect_models), which happens during
                    # registry load -- before any data file is parsed. So
                    # it is always already there, and flagging it would
                    # push every report's binding_model_id and every
                    # ir.model.access row into a false positive.
                    continue
