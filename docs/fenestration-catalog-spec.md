...
| 14 | Profile rate unit — **confirmed by the client** | Chawla rates are **PKR per running foot**, stock lengths **14 / 16 / 18 ft**. The melt's old `uom_note` called this unverified; that wording was stale and has been corrected. Still to be had **in writing from Chawla**. | If it turns out to be per metre, every profile cost is out by 3.28× — one formula, but reprice every open quote. |
| 15 | Drawing format in PDFs | Reports embed a **PNG** rasterised in the browser, because Odoo 19 renders PDFs via wkhtmltopdf and its inline-SVG support is unreliable. The SVG is stored alongside it. | If PDFs ever move to an HTML renderer, switch the reports to the stored SVG; nothing else changes. |
