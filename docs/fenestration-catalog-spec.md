...
| 14 | **Position product is a Service** **[revisit]** | 'Fenestration Position' was `consu`, which since Odoo 17 means Goods, so confirming a quote ran procurement and failed: *"No rule has been found to replenish 'Fenestration Position' in 'Inter-warehouse transit'"*. There is no production or installation flow for windows yet, so a position is work that is quoted and invoiced, not goods shipped. Invoiced on **ordered** quantities, since there is no delivery to measure. | When production/installation exists it may become Goods again with a real route. The separate data issue below should be fixed regardless. |
| 15 | **A customer whose delivery location is the transit location** | `stock.warehouse._update_partner_data` rewrites `property_stock_customer` AND `property_stock_supplier` to the company's *Inter-warehouse transit* for whichever partner is a warehouse's address — by design, since shipping to your own warehouse is an internal move. If a quote's customer is that same partner, `sale_stock._get_location_final()` returns transit and **any** Goods product fails to confirm, not just ours. | Use a real customer partner on quotes, or reset that partner's Customer Location to Partner Locations / Customers, or give the warehouse a different address partner. |
| 16 | Profile rate unit — **confirmed by the client** | Chawla rates are **PKR per running foot**, stock lengths **14 / 16 / 18 ft**. The melt's old `uom_note` called this unverified; that wording was stale and has been corrected. Still to be had **in writing from Chawla**. | If it turns out to be per metre, every profile cost is out by 3.28× — one formula, but reprice every open quote. |
| 17 | Drawing format in PDFs | Reports embed a **PNG** rasterised in the browser, because Odoo 19 renders PDFs via wkhtmltopdf and its inline-SVG support is unreliable. The SVG is stored alongside it. | If PDFs ever move to an HTML renderer, switch the reports to the stored SVG; nothing else changes. |
